import { Flavor, ConfigApi, EntityMetadata, EntityOrmField, fail, setOpts, PartialOrNull, OptsOf, Changes, newChangesProxy, Lens, loadLens, LoadHint, Loaded, isLoaded, ValueFilter, EntityFilter, FilterOf, ValueGraphQLFilter, EntityGraphQLFilter, GraphQLFilterOf, OrderBy, Collection, hasMany, setField, cleanStringValue } from 'joist-orm';
import { LargePublisher, Publisher, newLargePublisher, PublisherFields, PublisherOpts, PublisherIdsOpts, PublisherFilter, PublisherGraphQLFilter, PublisherOrder, largePublisherMeta, Critic, CriticId, criticMeta } from './entities';
...
import { Context } from 'src/context';
    ...
    export interface LargePublisherFields extends PublisherFields {
      id: { kind: "primitive"; type: number; unique: true; nullable: false };country: { kind: "primitive"; type: string; unique: false; nullable: undefined };
    }

    export interface LargePublisherOpts extends PublisherOpts {
      country?: string | null;critics?: Critic[];
    }
    ...
    export interface LargePublisherFilter extends PublisherFilter {
      country?: ValueFilter<string, null>;critics?: EntityFilter<Critic, CriticId, FilterOf<Critic>, null | undefined>;
    }

    export interface LargePublisherGraphQLFilter extends PublisherGraphQLFilter {
      country?: ValueGraphQLFilter<string>;critics?: EntityGraphQLFilter<Critic, CriticId, GraphQLFilterOf<Critic>, null | undefined>;
    }
    ...
    export abstract class LargePublisherCodegen extends Publisher {
      ...
      populate<const H extends LoadHint<LargePublisher>>(opts: { hint: H, forceReload?: boolean }): Promise<Loaded<LargePublisher, H>>;
      populate<const H extends LoadHint<LargePublisher>, V>(hint: H, fn: (p: Loaded<LargePublisher, H>) => V): Promise<V>;
      populate<const H extends LoadHint<LargePublisher>, V>(opts: { hint: H, forceReload?: boolean }, fn: (p: Loaded<LargePublisher, H>) => V): Promise<V>;
