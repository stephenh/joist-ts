import { Flavor, ConfigApi, EntityMetadata, EntityOrmField, fail, setOpts, PartialOrNull, OptsOf, Changes, newChangesProxy, Lens, loadLens, LoadHint, Loaded, isLoaded, BaseEntity, ValueFilter, EntityFilter, FilterOf, ValueGraphQLFilter, EntityGraphQLFilter, GraphQLFilterOf, OrderBy, newRequiredRule, Collection, hasMany, LargeCollection, hasLargeMany, setField, cleanStringValue } from 'joist-orm';
import { PublisherGroup, newPublisherGroup, publisherGroupMeta, Publisher, PublisherId, publisherMeta, Critic, criticMeta } from './entities';
...
import { Context } from 'src/context';
    ...
    export interface PublisherGroupFields  {
      id: { kind: "primitive"; type: number; unique: true; nullable: false };name: { kind: "primitive"; type: string; unique: false; nullable: undefined };createdAt: { kind: "primitive"; type: Date; unique: false; nullable: never };updatedAt: { kind: "primitive"; type: Date; unique: false; nullable: never };
    }

    export interface PublisherGroupOpts  {
      name?: string | null;publishers?: Publisher[];
    }
    ...
    export interface PublisherGroupFilter  {
      id?: ValueFilter<PublisherGroupId, never>;name?: ValueFilter<string, null>;createdAt?: ValueFilter<Date, never>;updatedAt?: ValueFilter<Date, never>;publishers?: EntityFilter<Publisher, PublisherId, FilterOf<Publisher>, null | undefined>;
    }

    export interface PublisherGroupGraphQLFilter  {
      id?: ValueGraphQLFilter<PublisherGroupId>;name?: ValueGraphQLFilter<string>;createdAt?: ValueGraphQLFilter<Date>;updatedAt?: ValueGraphQLFilter<Date>;publishers?: EntityGraphQLFilter<Publisher, PublisherId, GraphQLFilterOf<Publisher>, null | undefined>;
    }

    export interface PublisherGroupOrder  {
      id?: OrderBy;name?: OrderBy;createdAt?: OrderBy;updatedAt?: OrderBy;
    }

    export const publisherGroupConfig = new ConfigApi<PublisherGroup, Context>();

    publisherGroupConfig.addRule(newRequiredRule("createdAt"));publisherGroupConfig.addRule(newRequiredRule("updatedAt"));
    ...
    export abstract class PublisherGroupCodegen extends BaseEntity<EntityManager> {
      ...
      readonly critics: LargeCollection<PublisherGroup, Critic> = hasLargeMany(
        criticMeta,
        "critics",
        "group",
        "group_id"
      ...
      populate<const H extends LoadHint<PublisherGroup>>(opts: { hint: H, forceReload?: boolean }): Promise<Loaded<PublisherGroup, H>>;
      populate<const H extends LoadHint<PublisherGroup>, V>(hint: H, fn: (pg: Loaded<PublisherGroup, H>) => V): Promise<V>;
      populate<const H extends LoadHint<PublisherGroup>, V>(opts: { hint: H, forceReload?: boolean }, fn: (pg: Loaded<PublisherGroup, H>) => V): Promise<V>;
      populate<H extends LoadHint<PublisherGroup>, V>(hintOrOpts: any, fn?: (pg: Loaded<PublisherGroup, H>) => V): Promise<Loaded<PublisherGroup, H> | V> {
